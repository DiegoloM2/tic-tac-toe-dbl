...
class TCS34725():
    ...
    #Convert the luminance values for rgb to the rgb values in range 0 to 255
    def convertToRgb(self, lum, max_lum, min_lum):
        rgb = [0,0,0]
        for i in range (3):
            rgb[i] = int((lum[i] - min_lum[i])/(max_lum[i] - min_lum[i]) * 255)
            if rgb[i] < 0:
                rgb[i] = 0
            if rgb[i] > 255:
                rgb[i] = 255
